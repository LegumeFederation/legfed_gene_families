...
my ($in_align, $out_align, $log, $help, $verbose);
...
my $usage = <<EOS;
Usage: filter_align.pl -in ALIGN.fa
...
foreach my $seqobj ( $aln->each_seq() ) {
  ...
  foreach my $char ( split("", $seqobj->seq()) ) {
    ...
    $aln_col_scores[$col_idx]++ unless ($char =~ /-/);
    $col_idx++;
  }
}
...
foreach my $col ( @aln_cols ) {
  ...
  if ( $aln_col_scores[$col_idx]<$min_effective_depth ) {
    $col_idx++; 
    next 
  }
  else {
...
VERSIONS
sc = Steven Cannon
v01 picked up this script on-line somewhare, ca. 2010
